...
function buildTabs(){
  ...
  if (ME.rol === 'master') $('#purgaSection')?.classList.remove('hidden');  // TEMPORAL: quitar tras purga
}

/* TEMPORAL FASE 6.5 — purga total de datos de prueba para arrancar limpio (Excel de Reynaldo).
   ① lista TODO lo que se borraría (residentes + todos los movimientos) y lo que se conserva
   (master/admin). ② borra en orden pagos → familiares → jefes (el Worker respalda y limpia
   Auth de los registrados) y luego resetea el contador de folios REC- a -001. Solo master;
   el Worker revalida todo. QUITAR handler + UI + endpoint /finanzas/reset-folios tras usar. */
async function purgaFetchTodo(){
  const r = await authedFetch('/personas/listar', {});
  const personas = r.personas || [];
  const residentes = personas.filter(p => p.rol === 'residente');
  const staff = personas.filter(p => p.rol === 'admin' || p.rol === 'master');
  const snap = await db.collection('finanzas').get();
  const movs = []; snap.forEach(d => movs.push({ id: d.id, ...d.data() }));
  return {
    movs, staff,
    familiares: residentes.filter(p => p.jefeId),
    jefes: residentes.filter(p => !p.jefeId),
  };
}
$('#listarTodoBtn')?.addEventListener('click', async () => {
  const btn = $('#listarTodoBtn'), out = $('#purgaMsg');
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span>';
  try {
    const { movs, staff, familiares, jefes } = await purgaFetchTodo();
    const L = [];
    L.push(`SE BORRARÍAN:`);
    L.push(`• Movimientos de finanzas: ${movs.length}`);
    movs.slice().sort((a,b)=>(a.casa||'').localeCompare(b.casa||'','es',{numeric:true}))
      .forEach(m => L.push(`   – ${m.casa || '(sin casa)'} · ${m.folioRecibo || 'sin folio'} · ${money(m.monto)} · ${m.categoria || '—'} · ${m.tipo} · ${fmtTime(m.ts)}`));
    L.push(`• Residentes: ${familiares.length + jefes.length} (${jefes.length} jefes, ${familiares.length} familiares)`);
    jefes.slice().sort((a,b)=>(a.domicilio||'').localeCompare(b.domicilio||'','es',{numeric:true}))
      .forEach(j => L.push(`   – JEFE ${j.domicilio} / ${j.nombre}${j.registrado ? ' (registrado)' : ''}`));
    familiares.forEach(f => L.push(`   – familiar ${f.nombre}${f.registrado ? ' (registrado)' : ''}`));
    L.push('', `SE CONSERVAN (no se tocan): ${staff.length}`);
    staff.forEach(s => L.push(`   – ${s.nombre} · ${s.rol}`));
    L.push('', 'Además: reset del contador de folios REC- → siguiente recibo -001.');
    out.textContent = L.join('\n');
    toast('Listado generado', 'ok');
  } catch (e) { out.textContent = 'ERROR: ' + (e.message || e); toast('Error al listar', 'bad'); }
  finally { btn.disabled = false; btn.innerHTML = '🔎 Listar TODO lo que se borraría'; }
});
$('#borrarTodoBtn')?.addEventListener('click', async () => {
  const btn = $('#borrarTodoBtn'), out = $('#purgaMsg');
  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span>';
  const log = []; const pinta = () => out.textContent = log.join('\n');
  try {
    const { movs, familiares, jefes } = await purgaFetchTodo();
    log.push(`1) Borrando ${movs.length} movimiento(s)…`); pinta();
    for (const m of movs) {
      try { await authedFetch('/finanzas/borrar', { id: m.id }); log.push(`   ✅ ${m.folioRecibo || m.id} (${money(m.monto)})`); }
      catch (e) { log.push(`   ⚠️ ${m.folioRecibo || m.id} — ${e.message || 'error'}`); }
      pinta();
    }
    log.push('', `2) Borrando ${familiares.length} familiar(es)…`); pinta();
    for (const f of familiares) {
      try { await authedFetch('/personas/borrar', { id: f.id }); log.push(`   ✅ ${f.nombre}${f.registrado ? ' (+Auth)' : ''}`); }
      catch (e) { log.push(`   ⚠️ ${f.nombre} — ${e.message || 'error'}`); }
      pinta();
    }
    log.push('', `3) Borrando ${jefes.length} jefe(s)/casa(s)…`); pinta();
    for (const j of jefes) {
      try { await authedFetch('/personas/borrar', { id: j.id }); log.push(`   ✅ ${j.domicilio} / ${j.nombre}${j.registrado ? ' (+Auth)' : ''}`); }
      catch (e) { log.push(`   ⚠️ ${j.domicilio} / ${j.nombre} — ${e.message || 'error'}`); }
      pinta();
    }
    log.push('', '4) Reset del contador de folios…'); pinta();
    try { await authedFetch('/finanzas/reset-folios', {}); log.push('   ✅ folios reiniciados (siguiente recibo -001)'); }
    catch (e) { log.push('   ⚠️ reset folios — ' + (e.message || 'error')); }
    log.push('', 'Listo. Padrón y finanzas en cero.'); pinta();
    btn.textContent = 'Purga ejecutada ✓';
    toast('Purga ejecutada', 'ok');
    if (typeof refrescarPersonas === 'function') refrescarPersonas();
  } catch (e) { log.push('ERROR general: ' + (e.message || e)); pinta(); btn.disabled = false; btn.innerHTML = '🗑 Borrar TODO + reset folios'; toast('Error', 'bad'); }
});
